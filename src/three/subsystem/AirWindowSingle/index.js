...
import { air_window_only } from "@/assets/models";
import { Core3D } from "../..";
...
export class AirWindowSingle extends Subsystem {
    /** @param {Core3D} core*/
    constructor(core) {
        super(core);
        this.css2ds = [];
    ...
    async onEnter() {
        ...
        await loadGLTF(air_window_only,this.onProgress);
        await loadOBJ(air_window_only,this.onOBJProgress);
...
string} name
    ...
    onProgress = (gltf,name) => {
        ...
        if (name === "ground") {
            ...
            let geometry = new THREE.PlaneGeometry(3.52,25.2);
    ...
    onLoaded() {
        ...
        this.onRenderQueue.set(_BoringMachineSubsystem,this.update);
        this.box();
        this.setEquipmentState(1,"90"); // 两个风门关闭
    }
    box() {
    ...
    setEquipmentState(code,type) {
        ...
        let toValue = 0;
        if (this.fanner1.actionName.includes('开窗')) {
            toValue = 1;
        }
