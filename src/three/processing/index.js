...
import { edgeFade,water2,glass,glassNight,brightenNight,brighten } from "../../shader/functions";
...
function processingCommonModel(gltf,system,postprocess = undefined,preprocess = undefined) {
    ...
    model.traverse(child => {
        if (child instanceof Mesh) {
            ...
            if (lastName.includes("粗糙度")) {
                const roughness = parseFloat(lastName.split("粗糙度").pop());
                material.roughness = roughness;
                material.metalness = 1 - roughness;
            }

            if (material.name.includes("透明")) {
            ...
            // 如果发现效果不对，请在postprocess中自定义设置灯光参数
            if (child.name.indexOf("pointLight") !== -1) {
                const childName = child.name;
                const color = getOptionValue(childName,"&color="),
                    intensity = getOptionValue(childName,"&intensity="),
                    distance = getOptionValue(childName,"&distance=") || 5,
                    decay = getOptionValue(childName,"&decay=");
                ...
                pointLight.shadow.bias = -0.0066;
                system.add(pointLight);
            } else if (child.name.indexOf("spotLight") !== -1) {
                const childName = child.name;
                const color = getOptionValue(childName,"&color="),
                    intensity = getOptionValue(childName,"&intensity="),
                    distance = getOptionValue(childName,"&distance=") || 5,
                    decay = getOptionValue(childName,"&decay="),
                    angle = getOptionValue(childName,"&angle=") || Math.PI / 3,
                    x = getOptionValue(childName,"&x=") || 0,
                    y = getOptionValue(childName,"&y=") || 0,
                    z = getOptionValue(childName,"&z=") || 0,
                    tx = getOptionValue(childName,"&tx=") || 0,
                    ty = getOptionValue(childName,"&ty=") || 0,
                    tz = getOptionValue(childName,"&tz=") || 0;

                child.castShadow = false;
                const spotLight = new SpotLight(color,intensity,distance,angle,1,decay);
                spotLight.position.set(x,y,z);
                spotLight.target.position.set(tx,ty,tz);
                // spotLight.castShadow = true;
                spotLight.shadow.bias = -0.0066;
                spotLight.updateWorldMatrix(true,false);
                spotLight.target.updateWorldMatrix(true,false);

                system.add(spotLight);
            }
